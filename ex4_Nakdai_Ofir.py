...
# ID of  student: 318382827
# First and Last Names of student: Ofir Nakdai
    ...
    *** Mandatory structure ***
    ...
    def fit(self, dataset_df):
        ...
        *** This method will be called exactly once during evaluation. See the main section for details ***
        ...
        #numerical_columns = [ 'Transportation expense', 'Weight', 'Height', 'Service time'] # There are more - what else?
        ...
        self.res_distance_fill = dataset_df['Residence Distance'].median
        ...
        categorical_transformer = OneHotEncoder(drop=None, sparse=False, handle_unknown='infrequent_if_exist')
    ...
    def transform(self, df):
        ...
        """
        df['Education'].fillna('High school')
        df['Pet'].fillna(0)
        df['Smoker'].fillna('No')
        new_df = pd.DataFrame(self.transformer.transform(df))
        return new_df
...
def train_model(processed_X, y):
    ...
    #model = DecisionTreeClassifier(criterion= 'log_loss', max_depth=5 ,random_state=0)
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        sum_train += test_score
    print("test:", sum_test/50)
    print('train:', sum_train/50)
